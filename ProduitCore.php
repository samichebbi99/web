...
function ajouterProduit ($produit){
        $sql="insert into produit (reference,nom_produit,description,prix,id_categorie) values (:reference,:nom_produit,:description,:prix,:id_categorie)";
    ...
    function supprimerProduit($reference){
        ...
        function supprimerProduit1($id_categorie){
        $sql="DELETE FROM produit where id_categorie=:id_categorie";
        $db = config::getConnexion();
        $req=$db->prepare($sql);
        $req->bindValue(':id_categorie',$id_categorie);
        try{
...
try{        
        ...
        $prix=$produit->getprix();
        $id_categorie=$produit->getid_categorie();
        $datas = array(':reference'=>$reference, ':reference'=>$reference, ':nom_produit'=>$nom_produit,':description'=>$description,':prix'=>$prix,':id_categorie'=>$id_categorie);
        $req->bindValue(':reference',$reference);
        $req->bindValue(':reference',$reference);
        $req->bindValue(':nom_produit',$nom_produit);
        $req->bindValue(':description',$description);
